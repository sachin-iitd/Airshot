...
def parse_args():
    ...
    parser.add_argument("--interpol",type=int,default=1)    # 0 is forecasting mode
    parser.add_argument("--locid",type=int,default=4)
...
args = parse_args()
mode_t = ['AB','AC'][args.interpol]
mode_p = ['CD','D'][args.interpol]
pm = 'cpm'
...
def rename_cols(data):
    data.rename(
        columns={'dateTime': 'time', 'lat': 'latitude', 'long': 'longitude', pm: 'PM25_Concentration',
                 'pm10': 'PM10_Concentration'}, inplace=True)

def return_data_0(train_input, test_input, with_scaling):
    train_output = np.array(train_input[pm])
    train_input = train_input[['dateTime','lat','long']]
    test_output = np.array(test_input[pm])
...
def proc_file(locid=0):
    mfile = '{}_{}{}_{}{}_{}{}.metrics.csv'.format(algo, ['delhi','canada','canada','usa','kolkata'][locid], locid if locid in [1,2] else '', mode_t, 'T' if args.temporal_scaling else '', mode_p, '_st' if args.spatiotemporal>0 else '_20' if args.spatiotemporal<0 else '') if args.mfile is None else args.mfile
    f = open(mfile, 'a')
...
def proc_kolkata():
    f = proc_file()
    for nTestStartDay,nTotalDays in [(args.train_days+1,31+6),(31+8+args.train_days,62)]:
        for day in range(nTestStartDay, nTotalDays+1):
            date = []
            for i in range(args.train_days,-1,-1):
